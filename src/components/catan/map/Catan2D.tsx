...
import { COLORS, HARBOR_ICONS, STORE_ICONS } from "@/config/constants/ui";
...
> {
    ...
    updateScale = () => {
        ...
        if (outer) {
            console.log("outer", outer.height);
            const scaleY = outer.height / OriginalHeight;

            this.setState({ scale: scaleY }); // Prevent upscaling
        }
    };
                ...
                >
                    <div style={{ translate: `-215px -322.5px` }}>
                        ...
                        {Settlements.map(({ x, y }: any, index: number) => {
                            ...
                            const used =
                                settlements.has(index) && !cities.has(index);
                            ...
                            const opacity =
                                used && mapState !== "picking vertex upgrade"
                                    ? "ff"
                            ...
                            const className =
                                mapState === "picking vertex" && !used && allow
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() => {
                                        console.log("picked vertex:", index);
                                        this.state.events.emit(
                                            "picked vertex",
                                            index
                                        );
                        ...
                        {Settlements.map(({ x, y }: any, index: number) => {
                            ...
                            const color = `${baseColor}${opacity}`;
                            const className =
                                mapState === "picking vertex upgrade" && !used
                                    ? "animate-pulse hover:scale-95 hover:animate-none cursor-pointer"
                                    : "pointer-events-none";
                        ...
                        {Harbors.map(({ x, y }: any, index: number) => {
                            ...
                            const { icon, name } = HARBOR_ICONS[materialIndex];
                            const color =
                                materialIndex < 5
